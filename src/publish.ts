...
function validatePluginProject(): PluginConfig {
  ...
  if (!pluginConfig.pluginName) {
    throw new Error('plugin.json中缺少pluginName字段')
  }
...
export async function publish(): Promise<void> {
  ...
  try {
    ...
    console.log(green(`✓ 插件: ${pluginConfig.pluginName} (${pluginConfig.name})`))
    ...
    // 9. 创建PR
    const prTitle = `Add/Update plugin: ${pluginConfig.pluginName}`
    const prBody = `## 插件信息

**插件名称**: ${pluginConfig.pluginName}
...
此PR包含${commits.length}个提交，添加/更新插件 \`${pluginConfig.pluginName}\`。
...
`
    ...
    console.log(green('=' + '='.repeat(60)))
    console.log()
    console.log(cyan(`📦 插件: ${pluginConfig.pluginName}`))
